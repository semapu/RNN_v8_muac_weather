    ...
    for day, start_regulation, end_regulation, volume in zip(list_dates_with_regulations,
        ...
        for i in range(0, num_additional_samples_per_day):
            ...
            for start_reg, end_reg, vol in zip(timestamps_start_for_that_day_with_regulations,
                ...
                # Check if the sectors volumes are coincident
                if vol == volume:
...
def extract_features_from_list_days_and_timestamps(list_days,
    ...
    # Sanity check: Ensuring the list "sector_name" has the correct lenlist_days_from_regulationsgth
    ...
    for day, start, end, sector_name in zip(list_days, start_time_samples, end_time_samples, traffic_volume):
        ...
        X[counter_day][:][:, 1] = tmp_capacity[starting_index_inside_day:end_index_inside_day]
        ...
        X[counter_day][:][:, 2] = oc_atDate[starting_index_inside_day:end_index_inside_day]

        """ Entry count 20/20 """
        # Filtering the datestamp and the sector
        ec_20_20_atDate = ec_20_20[(ec_20_20.iloc[:, 0] == day) & (ec_20_20.iloc[:, 2] == 'EDYY' + sector_name)].values[0][5:-1]
        ...
        for i in range(0, gap_before_start_time + gap_after_start_time):
            ...
            X[counter_day][:][i, 3] = ec_20_20_atDate[index_value]
        ...
        ec_60_20_atDate = ec_60_20[(ec_60_20.iloc[:, 0] == day) & (ec_60_20.iloc[:, 2] == 'EDYY' + sector_name)].values[0][5:-1]

        for i in range(0, gap_before_start_time + gap_after_start_time):
            # ec_60_20 has a frequency of 20 minutes
            index_value = math.floor((starting_index_inside_day + i) / 20)
            X[counter_day][:][i, 4] = ec_60_20_atDate[index_value]
        ...
        """ Workload_1_1 """
        # Filtering the timestamps and the sector
        workload_at_date = workload_1_1[(workload_1_1.iloc[:, 0] == day) & (workload_1_1.iloc[:, 2] == 'EDYY' + sector_name)].values[0][5:-1]

        # Sanity check - Ensure only one row is obtained after the filtering
        if len(workload_at_date) == 0:
            sys.exit("No workload (1 1) were found for the date: " + str(day))

        X[counter_day][:][:, 5] = workload_at_date[starting_index_inside_day:end_index_inside_day]

        """ Workload_20_20 """
        # Filtering the timestamp and the sector
        workload_20_20_atDate = workload_20_20[(workload_20_20.iloc[:, 0] == day) & (workload_20_20.iloc[:, 2] == 'EDYY' + sector_name)].values[0][5:-1]

        for i in range(0, gap_before_start_time + gap_after_start_time):
            # ec_20_20 has a frequency of 20 minutes
            index_value = math.floor((starting_index_inside_day + i) / 20)
            X[counter_day][:][i, 6] = workload_20_20_atDate[index_value]
        ...
        #
        # X[counter_day][:][:, 7] = num_conflicts_at_date[starting_index_inside_day:end_index_inside_day]

        """ Climbing_1_1 """
        # Filtering the timestamps and the sector
        climbing_at_date = climbing_1_1[(climbing_1_1.iloc[:, 0] == day) & (climbing_1_1.iloc[:, 2] == 'EDYY' + sector_name)].values[0][5:-1]

        # Sanity check - Ensure only one row is obtained after the filtering
        if len(climbing_at_date) == 0:
            sys.exit("No climbing were found for the date: " + str(day))

        X[counter_day][:][:, 8] = climbing_at_date[starting_index_inside_day:end_index_inside_day]

        """ Cruising_1_1 """
        # Filtering the timestamps and the sector
        cruising_at_date = cruising_1_1[(cruising_1_1.iloc[:, 0] == day) & (cruising_1_1.iloc[:, 2] == 'EDYY' + sector_name)].values[0][5:-1]

        # Sanity check - Ensure only one row is obtained after the filtering
        if len(climbing_at_date) == 0:
            sys.exit("No cruising time were found for the date: " + str(day))

        X[counter_day][:][:, 9] = cruising_at_date[starting_index_inside_day:end_index_inside_day]

        """ Descending_1_1 """
        # Filtering the timestamps and the sector
        descending_at_date = descending_1_1[(descending_1_1.iloc[:, 0] == day) & (descending_1_1.iloc[:, 2] == 'EDYY' + sector_name)].values[0][5:-1]

        # Sanity check - Ensure only one row is obtained after the filtering
        if len(workload_at_date) == 0:
            sys.exit("No descending time were found for the date: " + str(day))

        X[counter_day][:][:, 10] = descending_at_date[starting_index_inside_day:end_index_inside_day]
